...
public sealed class ThemeRepository : SqliteRepositoryBase, IThemeRepository
{
    ...
    public async Task<ThemeUsage> GetUsageAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            SELECT
              (SELECT COUNT(*) FROM ReferenceTheme WHERE ThemeId=$id),
              (SELECT COUNT(DISTINCT mr.MessageId)
               FROM ReferenceTheme rt
               INNER JOIN MessageReference mr ON mr.ReferenceId=rt.ReferenceId
               WHERE rt.ThemeId=$id);
            """;
        command.Parameters.AddWithValue("$id", id);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        if (!await reader.ReadAsync(cancellationToken)) return new ThemeUsage(0, 0);
        return new ThemeUsage(reader.GetInt32(0), reader.GetInt32(1));
    }
