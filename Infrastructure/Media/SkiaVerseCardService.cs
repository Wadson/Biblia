...
public sealed class SkiaVerseCardService(IAppPaths paths, INatureMediaService media) : IVerseCardService
{
    public async Task<VerseCardResult> RenderAsync(VerseCardRequest request, CancellationToken cancellationToken = default)
    {
        ...
        using var image = SKImage.FromBitmap(bitmap); using var data = image.Encode(SKEncodedImageFormat.Png, 100); await using var stream = File.Create(file); data.SaveTo(stream); await stream.FlushAsync(cancellationToken); return new(file, width, height, "image/png");
    }
    private static void DrawBackground(SKCanvas canvas, int width, int height, NaturePhoto photo, string? path)
    {
        if (path is not null && File.Exists(path)) { using var image = SKBitmap.Decode(path); var scale = Math.Max((float)width / image.Width, (float)height / image.Height); var sourceWidth = width / scale; var sourceHeight = height / scale; var source = new SKRect((image.Width-sourceWidth)/2,(image.Height-sourceHeight)/2,(image.Width+sourceWidth)/2,(image.Height+sourceHeight)/2); canvas.DrawBitmap(image, source, new SKRect(0,0,width,height), new SKSamplingOptions(SKFilterMode.Linear, SKMipmapMode.None)); return; }
    ...
    private static SKTypeface LoadTypeface(string resource) { using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resource) ?? throw new InvalidOperationException("Fonte do card não encontrada."); return SKTypeface.FromStream(stream); }
