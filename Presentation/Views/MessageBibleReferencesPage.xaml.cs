...
public partial class MessageBibleReferencesPage : ContentPage, IQueryAttributable
{
 ...
 private void OnViewModelPropertyChanged(object? sender,PropertyChangedEventArgs e){MainThread.BeginInvokeOnMainThread(()=>{if(e.PropertyName==nameof(MessageReferencesViewModel.SelectedVersion)&&_viewModel.SelectedVersion is not null)VersionsList.ScrollTo(_viewModel.SelectedVersion,position:ScrollToPosition.Center,animate:true);else if(e.PropertyName==nameof(MessageReferencesViewModel.SelectedBook)&&_viewModel.SelectedBook is not null)BooksList.ScrollTo(_viewModel.SelectedBook,position:ScrollToPosition.Center,animate:true);else if(e.PropertyName==nameof(MessageReferencesViewModel.SelectedChapter)&&_viewModel.SelectedChapter>0)ChaptersList.ScrollTo(_viewModel.SelectedChapter,position:ScrollToPosition.Center,animate:true);});}
}
